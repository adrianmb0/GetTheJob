# GetTheJob -- AI Job Search Pipeline
...
- `{"status": "update-available", "local": "1.0.0", "remote": "1.1.0", "changelog": "..."}` → tell the user:
  > "get-the-job update available (v{local} → v{remote}). Your data (CV, profile, tracker, reports) will NOT be touched. Want me to update?"
  If yes → run `node update-system.mjs apply`. If no → run `node update-system.mjs dismiss`.
- `{"status": "up-to-date"}` → say nothing
- `{"status": "dismissed"}` → say nothing
- `{"status": "offline"}` → say nothing

The user can also say "check for updates" or "update get-the-job" at any time to force a check.
...
|---------|------------------------|-------------|
| `/get-the-job` | `/get-the-job` | Show menu or evaluate JD with args |
| `/get-the-job-pipeline` | `/get-the-job pipeline` | Process pending URLs from inbox |
| `/get-the-job-evaluate` | `/get-the-job oferta` | Evaluate job offer (A-F scoring) |
| `/get-the-job-compare` | `/get-the-job ofertas` | Compare and rank multiple offers |
| `/get-the-job-contact` | `/get-the-job contacto` | LinkedIn outreach (find contacts + draft) |
| `/get-the-job-deep` | `/get-the-job deep` | Deep company research |
| `/get-the-job-pdf` | `/get-the-job pdf` | Generate ATS-optimized CV |
| `/get-the-job-latex` | `/get-the-job latex` | Export CV as LaTeX/Overleaf .tex |
| `/get-the-job-training` | `/get-the-job training` | Evaluate course/cert against goals |
| `/get-the-job-project` | `/get-the-job project` | Evaluate portfolio project idea |
| `/get-the-job-tracker` | `/get-the-job tracker` | Application status overview |
| `/get-the-job-apply` | `/get-the-job apply` | Live application assistant |
| `/get-the-job-scan` | `/get-the-job scan` | Scan portals for new offers |
| `/get-the-job-batch` | `/get-the-job batch` | Batch processing with parallel workers |
| `/get-the-job-patterns` | `/get-the-job patterns` | Analyze rejection patterns and improve targeting |
| `/get-the-job-followup` | `/get-the-job followup` | Follow-up cadence tracker |

**Note:** OpenCode commands invoke the same `.claude/skills/get-the-job/SKILL.md` skill used by Claude Code. The `modes/*` files are shared between both platforms.
...
| `/get-the-job` | `/get-the-job` | Show menu or evaluate JD with args |
| `/get-the-job-pipeline` | `/get-the-job pipeline` | Process pending URLs from inbox |
| `/get-the-job-evaluate` | `/get-the-job oferta` | Evaluate job offer (A-G scoring) |
| `/get-the-job-compare` | `/get-the-job ofertas` | Compare and rank multiple offers |
| `/get-the-job-contact` | `/get-the-job contacto` | LinkedIn outreach (find contacts + draft) |
| `/get-the-job-deep` | `/get-the-job deep` | Deep company research |
| `/get-the-job-pdf` | `/get-the-job pdf` | Generate ATS-optimized CV |
...
> - Run `/get-the-job scan` (or `/get-the-job-scan` if using OpenCode) to search portals
> - Run `/get-the-job` to see all commands
>
...
If the user accepts, use the `/loop` or `/schedule` skill (if available) to set up a recurring `/get-the-job scan` (or `/get-the-job-scan` if using OpenCode). If those aren't available, suggest adding a cron job or remind them to run `/get-the-job scan` (or `/get-the-job-scan` if using OpenCode) periodically.
...
- **NEVER expose internal codenames in candidate-facing materials.** Internal project codenames, working titles, and unreleased product names from the user's current or past employers must be redacted from every CV, cover letter, application form answer, LinkedIn message, or any other text the user sends externally. Refer to internal projects descriptively only (e.g., "an in-call AI copilot for call-center workflows" rather than the internal codename). Internal-only files (`reports/`, `article-digest.md`, `cv.md`, `modes/_profile.md`, `interview-prep/`) may keep codenames. Before declaring CV/cover-letter generation complete, scan the output for known codenames and abort regeneration if any leak through.
...
| State | When to use |
|-------|-------------|
| `Shortlisted` | User flagged from triage; intent to apply but no full eval yet. Promotes to `Evaluated` when `apply` runs. |
