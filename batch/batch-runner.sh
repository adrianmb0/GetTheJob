...
# get-the-job batch runner — standalone orchestrator for claude -p workers
...
usage() {
  cat <<'USAGE'
get-the-job batch runner — process job offers in batch via claude -p workers
...
Options:
  ...
  --prompt FILE        Prompt template file (default: batch-prompt.md)
  --model NAME         Override claude model (e.g. claude-sonnet-4-6). Default: Max default.
...
# Resolve PROMPT_FILE relative to BATCH_DIR if not absolute and not found as given
if [[ "$PROMPT_FILE" != /* && ! -f "$PROMPT_FILE" && -f "$BATCH_DIR/$PROMPT_FILE" ]]; then
  PROMPT_FILE="$BATCH_DIR/$PROMPT_FILE"
fi
...
process_offer() {
  ...
  # Launch claude -p worker (uses default model from Claude Max subscription unless --model set)
  local exit_code=0
  local -a model_args=()
  if [[ -n "$MODEL" ]]; then
...
main() {
  ...
  echo "=== get-the-job batch runner ==="
  echo "Parallel: $PARALLEL | Max retries: $MAX_RETRIES"
  echo "Prompt:   $(basename "$PROMPT_FILE")"
  echo "Model:    ${MODEL:-<Max default>}"
