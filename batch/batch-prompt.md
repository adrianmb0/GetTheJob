...
PDFs are generated only on demand when the user enters `apply` mode for a specific role. In this step:

- Skip PDF generation entirely.
- Set `pdf_emoji = "❌"` in the tracker TSV.
- The PDF column flips to `✅` later, when `apply` mode runs.

If the user explicitly requests `--with-pdf` for a batch run, use `batch/batch-prompt-with-pdf.md` (which retains the full PDF pipeline reference).
