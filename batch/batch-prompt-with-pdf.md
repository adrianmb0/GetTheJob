# get-the-job Batch Worker — Evaluación Completa + PDF + Tracker Line
...
**PDF:** get-the-job/output/cv-candidate-{company-slug}-{{DATE}}.pdf
**Batch ID:** {{ID}}
...
### Paso 4 — DEFERRED: Do NOT generate PDF in batch

**PDFs are no longer generated during batch evaluation.** They burn tokens for every offer regardless of whether the user ever applies. PDFs are generated only on demand when the user enters `apply` mode for a specific role.

Behavior in this step:
- Skip PDF generation entirely.
- Set `pdf_emoji = "❌"` in the tracker TSV.
- The PDF column flips to `✅` later, when `apply` mode runs and produces the tailored CV.

**For reference, the original PDF pipeline is preserved below in case the user explicitly requests batch PDFs (uncommon — only run when the user passes `--with-pdf` or equivalent override).**

#### (Reference only — DO NOT EXECUTE unless user explicitly asks for PDFs)
