...
- If the score is >= 4.0, end the report with a one-line note: *"PDF + cover letter will be generated when you say `apply to {company}`."*
- If the user explicitly asks for the PDF in this same turn (e.g. "generate the PDF anyway", "I'm going to apply now"), then run `modes/pdf.md` and update the tracker row to `PDF: ✅`.
