...
> "No active applications to follow up on. Apply to some roles first with `/get-the-job` and come back when they're aging."
...
- Suggest the user run `/get-the-job contacto {company}` to find the right person first
...
> - Trying a different contact via `/get-the-job contacto`
...
**To:** {email or "No contact found — run `/get-the-job contacto` first"}
